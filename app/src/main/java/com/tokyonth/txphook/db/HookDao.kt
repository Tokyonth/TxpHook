...
interface HookDao {
    ...
    @Transaction
    @Query("SELECT * FROM ${Constants.DATABASE_TABLE_RULE_NAME} WHERE hookName = :name AND pkgName = :packageName")
    suspend fun queryRule(name: String, packageName: String): HookRule?
